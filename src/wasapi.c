...
#define soundio_IID_IAUDIOCLIENT                      (soundio_IID_IAudioClient)
#define soundio_IID_IMMENDPOINT                       (soundio_IID_IMMEndpoint)
#define soundio_IID_IAUDIOCLOCKADJUSTMENT             (soundio_IID_IAudioClockAdjustment)
#define soundio_IID_IAUDIOSESSIONCONTROL              (soundio_IID_IAudioSessionControl)
#define soundio_IID_IAUDIORENDERCLIENT                (soundio_IID_IAudioRenderClient)
#define soundio_IID_IMMDEVICEENUMERATOR               (soundio_IID_IMMDeviceEnumerator)
#define soundio_IID_IAUDIOCAPTURECLIENT               (soundio_IID_IAudioCaptureClient)
#define soundio_IID_ISIMPLEAUDIOVOLUME                (soundio_IID_ISimpleAudioVolume)
#define soundio_CLSID_MMDEVICEENUMERATOR              (soundio_CLSID_MMDeviceEnumerator)
...
static int refresh_devices(struct SoundIoPrivate *si) {
    ...
    for (int device_i = 0; device_i < device_count; device_i += 1) {
        ...
        if (FAILED(hr = IMMDevice_QueryInterface(rd.mm_device, soundio_IID_IMMENDPOINT, (void**)&rd.endpoint))) {
        ...
        if (FAILED(hr = IMMDevice_Activate(rd.mm_device, soundio_IID_IAUDIOCLIENT,
...
static void device_thread_run(void *arg) {
    ...
    HRESULT hr = CoCreateInstance(soundio_CLSID_MMDEVICEENUMERATOR, NULL,
            CLSCTX_ALL, soundio_IID_IMMDEVICEENUMERATOR, (void**)&siw->device_enumerator);
    if (FAILED(hr)) {
...
static int outstream_do_open(struct SoundIoPrivate *si, struct SoundIoOutStreamPrivate *os) {
    ...
    if (FAILED(hr = IMMDevice_Activate(dw->mm_device, soundio_IID_IAUDIOCLIENT,
    ...
    if (outstream->name) {
        if (FAILED(hr = IAudioClient_GetService(osw->audio_client, soundio_IID_IAUDIOSESSIONCONTROL,
    ...
    if (FAILED(hr = IAudioClient_GetService(osw->audio_client, soundio_IID_IAUDIORENDERCLIENT,
                    (void **)&osw->audio_render_client)))
    {
        return SoundIoErrorOpeningDevice;
    }

    if (FAILED(hr = IAudioClient_GetService(osw->audio_client, soundio_IID_ISIMPLEAUDIOVOLUME,
...
static int instream_do_open(struct SoundIoPrivate *si, struct SoundIoInStreamPrivate *is) {
    ...
    if (instream->name) {
        if (FAILED(hr = IAudioClient_GetService(isw->audio_client, soundio_IID_IAUDIOSESSIONCONTROL,
    ...
    if (FAILED(hr = IAudioClient_GetService(isw->audio_client, soundio_IID_IAUDIOCAPTURECLIENT,
...
{
    if (IS_EQUAL_IID(riid, &IID_IUnknown) || IS_EQUAL_IID(riid, &soundio_IID_IMMNotificationClient)) {
        *ppv = client;
